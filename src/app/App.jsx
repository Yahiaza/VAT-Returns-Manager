...
export default function App(){
  ...
  return <div className="h-screen flex flex-col bg-[#f6efe6]" dir="rtl">
    <TitleBar/>
    <div className="flex flex-1 min-h-0">
    <aside className="w-[238px] shrink-0 bg-[#4a3028] text-white p-4 flex flex-col shadow-xl">
      ...
      <div className="space-y-1">{nav.map(([id,label,Icon])=><button key={id} onClick={()=>setPage(id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition ${page===id?'bg-[#f7e8eb] text-[#7f2638] shadow':'text-white/75 hover:bg-white/10 hover:text-white'}`}><Icon size={18}/>{label}</button>)}</div>
    ...
    {toast&&<div className="fixed left-6 bottom-6 bg-[#4a3028] text-white px-5 py-3 rounded-xl shadow-2xl z-50 flex items-center gap-2"><Bell size={18}/>{toast}</div>}
    </div>
  </div>
}
