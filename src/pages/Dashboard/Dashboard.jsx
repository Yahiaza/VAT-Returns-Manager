...
export default function Dashboard({data,period,setPage}){
  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-3 gap-5"><div className="card p-5 col-span-2"><div className="flex items-center justify-between mb-4"><div><div className="font-bold">إقرار {period.label}</div><div className="text-sm text-slate-400 mt-1">حالة العمل والمتابعة</div></div><span className="badge bg-blue-50 text-blue-700"><CheckCircle2 size={14}/>{statuses[period.status]||period.status}{Number(period.is_locked)===1?' — مقفل':''}</span></div><div className="grid grid-cols-3 gap-4 text-sm"><Info label="الفروع التي بها إدخال" value={`${completed} / ${activeBranches.length}`}/><Info label="موعد الاستحقاق" value={period.due_date||'غير محدد'}/><Info label="النسبة الأساسية" value={`${period.tax_rate}%`}/></div><button className="btn btn-primary mt-5" onClick={()=>setPage('entry')}>{Number(period.is_locked)===1?'معاينة الإقرار':'فتح شاشة إدخال الفروع'}</button></div><div className="card p-5"><div className="font-bold mb-3">الوصول السريع</div><button onClick={()=>setPage('reports')} className="quick-link">عرض نموذج VAT النهائي</button><button onClick={()=>setPage('attachments')} className="quick-link mt-2">مرفقات الإقرار والسداد</button></div></div></>}
